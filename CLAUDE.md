...
- **リストの追加方針 (所有者、2026-09-26)**: シンガポールにあることの確証 (レジストリの country SG、または事業者自身の geofeed) があれば FACEIT 以外でも入れてよい。誤ったエントリーは消す。ただし汎用クラウド/CDN (AWS・Azure・Alibaba・DigitalOcean・Akamai・Google の GCP 以外など)、VPN 業者 (Packethub = NordVPN 基盤など)、IP リース/顧客ブロックは入れない。理由: 経由させる一般通信が増えるとノード停止中 (02:00〜19:00) にそれらが不通になり、送信転送も食う。出口ノード自身 (AWS) を含む範囲も不可。
- 2026-09-26 の変更: 103.5.12.0/22 (登録は OVH 香港 OVH-VKS-HKG1/2) を削除、139.99.0.0/16 を 139.99.0.0/17 に縮小 (139.99.128.0/17 は OVH Australia)。追加は Vultr SG (公式 geofeed の SG 16 プレフィクス全部)、Leaseweb SG (登録名 LSW-SG* / LSW-SIN* / LSW-AS-AP の自社ブロック)、SG.GS と傘下ブランド (HostSG、OneAsiaHost。116.251.212.0/23 は SGGS-MY なので除外)、GSL Networks の隣接 /24 と Streamline Servers、i3D.net SG (213.179.200.0/22、i3Dnet-sgsin1-*)、Gcore SG (GCL-CUSTOMER-SG など 17 ブロック。エニーキャスト/DNS と非 SG は除外)。計 150 CIDR。除外したもの: Datacamp AS212238 の顧客ブロック、Leaseweb/GSL の顧客・VPN ブロック、Gcore 上の Discord SG 音声 (66.22.220.0/23)、i3D 上の Ubisoft ブロック (国の記載なし)、Valve 103.28.54.0/24 (SDR リレーではない)、PhoenixNAP の残り (大半が国記載なしの共有ブロックで、直結のほうが速い事業者)。
